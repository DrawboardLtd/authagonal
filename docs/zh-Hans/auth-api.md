...
提交上来的验证码在查找之前会按 RFC 8628 §6.1 规范化：转为大写，并丢弃 31 字符验证码字母表之外的所有字符。`ABCD-EFGH`、`abcd-efgh`、`ABCDEFGH`、`ABCD EFGH`，以及粘贴时把连字符变成了破折号的情况，都是同一个验证码。连字符的存在只是为了让验证码更便于朗读。输入被限制为每个 subject 每分钟十次尝试（RFC 8628 §5.1）；第十一次返回 `429`。在默认的进程内限流器下该计数器是按节点计的，因此多副本部署还应在边缘侧强制执行该限制。
