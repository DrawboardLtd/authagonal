...
> **转储仍然会泄露什么。**「既无法重新计算也无法逆向还原」这句话对单个令牌成立，对整个索引并不成立。
> 有三项残留会保留下来，在依赖此机制之前值得先了解它们：
>
>   *（已修复。）* ~~**结构。** 前缀索引为每个前缀写入一行，因此一条记录的行数就等于被索引字段的长度。~~
>   现在每个被索引的值都会写入固定数量的行，不足部分用诱饵填充——任何查询都无法产生这些诱饵，转储也无法
>   将它们与真实前缀区分开。
> - **相等性与频率。** 令牌在设计上就是确定性的，这正是查找得以工作的原因，因此转储会显示哪些记录共享同
>   一个值，以及每个值有多常见。域名索引会按雇主对您的用户群分桶，这往往足以识别出具体的人，而无需还原出
>   地址。
> - **选择明文。** 既能读取存储*又*能促使值被索引的攻击者（注册一个账户、通过 SCIM 被预配）可以提交一个候
>   选值并查找它的令牌。无论密钥存放在哪里，这都能还原出任何可猜测的值——常见域名、常见名字——因为这个旁道
>   来自写入路径，而不是密码算法本身。
>
> 令牌化防御的是它被设计用来防御的场景：某人手里只有一份转储，别无其他，试图读取地址。剩下的两项残留恰恰
> 是注册旁道本来就会泄露的东西。如果它们不可接受，请让前缀索引表和域名索引表保持未配置状态——精确匹配查找
> 两者都不涉及——而不要假定 HMAC 已经覆盖了它们。
...
`IChangeWriter`（在 0.6.0 中由 `ITombstoneWriter` 更名而来）将每个变更行的键记录到专用的变更日志表，这样增量备份无需扫描实时表中未建索引的 `Timestamp` 列即可找到变更内容。删除操作对每个表都会捕获（实时行扫描无法看到已消失的行）；更新插入（upsert）则只对备份从日志读取而非扫描的那些表捕获。内置实现：`TableChangeWriter`（Azure Table Storage）、`DynamoChangeWriter`（DynamoDB）和 `SqlChangeWriter`（PostgreSQL / SQLite）。
