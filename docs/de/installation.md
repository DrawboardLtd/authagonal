...
Das Speicheranbieter-Paket ist austauschbar: `Authagonal.AzureProvider` für Azure Table Storage (die Standardverkabelung von `AddAuthagonal()`), oder `Authagonal.AwsProvider` für DynamoDB / S3 / Secrets Manager, siehe [AWS-Backend](#aws-backend) weiter unten.
