...
IdP-initiiertes SSO wird unterstützt. Der ACS-Endpunkt verarbeitet Antworten ohne `InResponseTo` (die Prüfung der Anfrage-ID wird bei unaufgeforderten Antworten übersprungen, aber die Einmaligkeit der Assertion-ID wird weiterhin durchgesetzt, siehe Sicherheit).
