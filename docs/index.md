...
OAuth 2.0 / OpenID Connect / SAML 2.0 authentication server for .NET, backed by pluggable storage: your own PostgreSQL or SQLite, or cloud-managed Azure Table Storage or AWS (DynamoDB / S3 / Secrets Manager).
...
- **Pluggable storage**: self-hosted PostgreSQL or SQLite (no cloud account, no emulator), or Azure Table Storage / AWS (DynamoDB / S3 / Secrets Manager) for low-cost serverless-friendly backends
