...
- **Exécutez derrière un proxy terminant le TLS, et déclarez-le.** Authagonal doit être placé derrière un reverse proxy / ingress qui termine le TLS (ou terminer le TLS lui-même). HSTS n'est émis que sur HTTPS et `/connect/*` refuse le texte en clair, le proxy doit donc transférer `X-Forwarded-Proto: https` — et cet en-tête est ignoré tant que vous n'avez pas défini `ForwardedHeaders:KnownNetworks` (ou `KnownProxies`) sur le CIDR ou l'adresse de votre proxy. Utilisez `["0.0.0.0/0", "::/0"]` si le proxy n'a pas d'adresse fixe et que rien d'autre ne peut atteindre le processus. `ForwardedHeaders:ForwardLimit` vaut `1` par défaut (ne faire confiance qu'au dernier saut).
...
- **Protégez les points d'accès internes.** Définissez `Cluster:Secret` pour que le point d'accès interne `/_internal/backchannel-logout` exige l'en-tête `X-Cluster-Secret` (comparé en temps constant). Sans secret, le point d'accès n'autorise **personne** et répond 404 : une adresse source n'est pas un credential, et la boucle locale est ce qu'un proxy inverse sur le même hôte présente pour chaque requête transférée. `Cluster:AllowLoopbackWithoutSecret` réadmet un pair boucle locale avant transfert, pour le développement local uniquement. Rien dans le produit livré n'appelle ce point d'accès, échouer fermé ne casse donc aucun flux propriétaire : définissez le secret si vous construisez dessus votre propre diffusion pod à pod.
