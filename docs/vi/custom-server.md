...
| `Oidc:Issuer` | Thường giống với `Issuer`. Tách riêng nếu URL nội bộ và bên ngoài khác nhau. |
...
|---|---|---|
| `IEmailService` | Gửi email xác minh và đặt lại mật khẩu | SendGrid (cần cấu hình) |
...
SPA đăng nhập mặc định hoạt động ngay lập tức, nhưng bạn có thể thay thế bằng ứng dụng React của riêng mình, nhập các component và API client từ gói npm `@drawboard/authagonal-login`.
