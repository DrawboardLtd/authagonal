...
SSO khởi tạo từ IdP được hỗ trợ — endpoint ACS xử lý các phản hồi không có `InResponseTo` (bỏ qua xác thực phát lại cho các phản hồi không được yêu cầu).
...
- **Ngăn chặn chuyển hướng mở:** RelayState (returnUrl) phải là đường dẫn gốc-tương đối (bắt đầu bằng `/`, không có scheme hoặc host)
