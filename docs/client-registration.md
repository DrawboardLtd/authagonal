...
What it does **not** constrain is the audience. RFC 7591 has no field for one, so a dynamically registered client always arrives with an empty `Audiences` list — which means "unset", not "deny everything" — and may therefore name any absolute URI as its `resource` at the authorization endpoint and receive a token carrying that value as `aud`. That is deliberate (the MCP authorization spec requires clients to name the MCP server as the resource, and an MCP client is a DCR client), and it makes the resource server responsible for authorizing on `scope` rather than on `iss` + `aud` + `sub`. Token exchange is the exception: there an empty `Audiences` denies outright, so a registered client cannot aim an exchanged token anywhere. See [Audiences and resource indicators](configuration#audiences-and-resource-indicators-rfc-8707).
