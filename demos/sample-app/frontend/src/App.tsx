...
import { login, logout, handleCallback, getStoredAuth, refreshAccessToken, needsRefresh, getConfig, type AuthState } from './auth';

type Tab = 'profile' | 'api' | 'token';
...
export default function App() {
  ...
  const [authServer, setAuthServer] = useState('http://localhost:8080');

  useEffect(() => {
    async function init() {
      try {
        const config = await getConfig();
        setAuthServer(config.authServer);
  ...
  const mfaUrl = `${authServer}/mfa-setup?backUrl=${encodeURIComponent(window.location.origin)}`;
          ...
          <div className="tab-content">
            {activeTab === 'profile' && (
              <div className="card">
                ...
                <div className="button-group">
                  <a href={mfaUrl} className="btn-secondary btn-link">MFA Settings</a>
