...
public sealed class LogHygieneTests : IAsyncDisposable
{
    ...
    /// <summary>
    /// Registration is anonymous too, and its log lines carried the address itself. They now carry the
    /// user id and the domain, and no local part in any form: application logs travel much further than
    /// the user store, and the id resolves to the account for anyone entitled to look it up.
    /// </summary>
    /// <remarks>
    /// This asserted the masked <c>l***@example.com</c> form when it was written. Two findings landed on
    /// these same lines — #245 (mask the address) and #307 (drop it, keep the id) — and #307's shape is
    /// strictly the stronger one, so the assertion moved with the code: no local part at all, masked or
    /// otherwise. The domain still goes through <see cref="LogSafe"/> because it is caller-controlled.
    /// </remarks>
    [Fact]
    public async Task Registration_DoesNotRecordTheFullAddress()
    {
        ...
        // Not the masked form either — the local part does not reach the log at all.
        Assert.DoesNotContain(_factory.LogSink.Messages,
            m => m.Contains("loguser", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(_factory.LogSink.Messages,
            m => m.Contains("l***@", StringComparison.Ordinal));
        // Non-vacuity: the line is still there, still diagnostic — id plus domain.
        Assert.Contains(_factory.LogSink.Messages,
            m => m.Contains("User registered:", StringComparison.Ordinal)
